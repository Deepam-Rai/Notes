...
3. Unable to handle sequential data properly.
	1. If during writing of sequential data, the write heads jumps somewhere then it's unable to get back the sequence.

----
# DNC
Differentiable Neural Computers addresses the limitations of NTM.  
...
$$\large M_t = M_{t-1} \circ (E-w_t^r e_t^T) + w_t^r v_t^T$$
...
By means of new data structure.  
...
$u_t$ is set to zeroes intitially, with usage it gets updated, how comes later,  
...
Explanation:  
$\phi_t[x] =$ just an $u_t$ index of an element, say $z$, which is actually a location in memory,  
$u_t[\phi_t[x]]]=$ denotes the usage of location $z$,  
$a_t[\phi_t[j]]=$ allocation weight of location $z$,  
$a_t[\phi_t[j]] = (1-u_t[\phi_t[j]])$ means more the usage less the allocation weight,  
multiplying it further with that summation further reduces the allocation weight for more used locations.  
